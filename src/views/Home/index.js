...
class Home extends Component {
  ...
  componentWillMount() {
    ...
    this
      .props
      .updateUser(getLocationStorage('user'));
    this
      .props
      .updateActiveUser(getLocationStorage('activeUser'));
  }
  ...
  handleLoginSubmit() {
    let {user} = this.props, {username, password} = this.state[this.state.active];
  ...
  handleRegisterSubmit() {
    let {user} = this.props, {email, mobile} = this.state[this.state.active];
  ...
  showErrorMessages(key, msg) {
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      if (this.state[this.state.active][key].validation.msg) {
        break;
      }
    }
  }
  ...
  showAllErrorMessages() {
    Object
      .keys(this.state[this.state.active])
      .map(field => {
  ...
  changeTab(url) {
    this.setState({
      active: (this.state.active == 'registerField')
        ? 'loginField'
        : 'registerField'
    });
  }
  ...
  renderRedirectionUrl() {
    ...
    return (
      <div className="text-align-center loginor-register">
        {firstLabel}
        <span
          className="nav-link"
          onClick={this
          .changeTab
          .bind(this)}>{urlLabel}</span>
      </div>
    )
  }
  ...
  render() {
    ...
    return (
      <div className="">
        <Header
          label={this.state.active == 'loginField'
          ? 'LOGIN TO APP'
          : 'REGISTER TO APP'}/>
        <div className="credential-block">
          <div className="credential-inner-block">
            <div className="nav">
              <div
                className={this.state.active == 'loginField'
                ? 'nav1-2'
                : 'nav2-2'}></div>
            </div>
            <form onSubmit={this
                ...
                .map((field, key) => {
                  ...
                  return (
                    <div className="field-block" key={key}>
                      <label>
                        {details.label}
                        {details['label_info'] && <span className="label-info">{details['label_info']}</span>}
                      </label>
                      {this.renderField(details, field)}
                      {(details.validation && details.validation.msg) && <div className="validation-error">
                        {details.validation.msg}
                      </div>}
                    </div>
                  )
                })}
              <div className="text-align-center credential-button">
                <button type="submit" className="button-btn">
                  {this.state.active == 'registerField'
                    ? 'REGISTER'
                    : 'LOGIN'}
                </button>
              </div>
...
function mapStateToProps(state) {
  return {activeUser: state.activeUser, user: state.user};
}
